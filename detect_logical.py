    ...
    def detect(self, name_list, img):
        ...
        with torch.no_grad():
            ...
            pred = non_max_suppression(pred, self.opt.conf_thres, self.opt.iou_thres,
                                       classes=[0], agnostic=self.opt.agnostic_nms)

            info_show = ""
            # 清空列表，每次检测重新记录人的信息
            name_list.clear()
            person_count = 0  # 每次检测前清零人数计数器
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        # 仅保留类别为人的检测结果
                        if int(cls) == 0:
                            name_list.append(label)  # 记录每个检测到的人的信息
                            single_info = plot_one_box2(xyxy, showimg, label=label,
                                                        color=self.colors[int(cls)], line_thickness=2)
                            info_show = single_info + "\n"
                            person_count += 1  # 更新人数计数器

            # 在输出信息中添加人数统计
            info_show += f"人数： {person_count}"
